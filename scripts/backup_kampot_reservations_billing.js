// One-off backup: exports Kampot branch's Reservation + Billing data to local JSON
// before it gets wiped for a from-scratch test pass.
//
...
//
// Run: node scripts/backup_kampot_reservations_billing.js
...
async function main() {
  ...
  if (!branches || branches.length === 0) throw new Error('No branch found matching "Kampot"')
  if (branches.length > 1) {
    console.log('Multiple branches matched "Kampot":')
  ...
  const outDir = path.join('backups', `kampot_reservations_billing_${timestamp}`)

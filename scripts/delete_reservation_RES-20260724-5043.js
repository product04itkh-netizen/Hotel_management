// Deletes reservation RES-20260724-5043 (Srae Ambel) and everything linked to
// it: line items, deposit receipts, check-in records, checkout inspections,
// invoices, payments, and the journal entries (+lines) tied to the reservation
// number OR any of its invoice numbers. Petty cash is UNLINKED, never deleted.
// Guest is left intact.
//
// Aborts if the reservation is missing or resolves to a different branch.
// Takes a pre-delete backup snapshot, deletes in dependency order, verifies.
//
// Run: node scripts/delete_reservation_RES-20260724-5043.js
...
const RESERVATION_NUMBER = 'RES-20260724-5043'
...
async function main() {
  ...
  if (!res) throw new Error(`${RESERVATION_NUMBER} not found for Srae Ambel branch`)
  ...
  const references = [res.reservation_number, ...invoiceNumbers]
  ...
  const { data: pettyCash } = await supabase.from('petty_cash_transactions').select('*').eq('reservation_id', res.id)
  ...
  const snapshot = {
    reservation: [res], reservation_line_items: lineItems, deposit_receipts: depositReceipts,
  ...
  if (pettyCash.length) {
    await supabase.from('petty_cash_transactions').update({ reservation_id: null, reservation_line_item_id: null }).eq('reservation_id', res.id)
  ...
  const { count: jeCount } = references.length ? await supabase.from('journal_entries').select('id', { count: 'exact', head: true }).in('reference', references).eq('branch_id', branch.id) : { count: 0 }
  console.log(`\nVerify — reservation remaining: ${resCount}, invoices remaining: ${invCount}, JEs remaining: ${jeCount}`)
  console.log(resCount === 0 && invCount === 0 && jeCount === 0 ? 'Delete complete.' : 'WARNING: something did not delete.')
}
